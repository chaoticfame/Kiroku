...
public class AnimeSection {
    ...
    public AnimeSection(DefaultListModel<String> model, Runnable onBack) {
        this.model = model;
        this.onBack = onBack;
        ...
        card.setBorder(BorderFactory.createTitledBorder(
                BorderFactory.createLineBorder(Color.decode("#3b82f6")),
                "Add New Anime",
        ...
        JLabel lblTitle = new JLabel("Anime Title:");
        ...
        JLabel lblProgress = new JLabel("Current Episode:");
        lblProgress.setForeground(Color.WHITE);
        lblProgress.setFont(new Font("SansSerif", Font.PLAIN, 13));

        spnProgress = new JSpinner(new SpinnerNumberModel(1, 1, 10000, 1));
    ...
    private void addEntry() {
        ...
        if (title.isEmpty()) {
            JOptionPane.showMessageDialog(panel, "Please enter an anime title.", "Missing Title", JOptionPane.WARNING_MESSAGE);
            return;
        }
        String status = (String) cmbStatus.getSelectedItem();
        int progress = (Integer) spnProgress.getValue();
        int rating = (Integer) spnRating.getValue();
        model.addElement("Anime • " + title + " • " + status + " • Ep " + progress + " • " + rating + "/10");
        clearForm();
    }
